...
#define	IS_IBUTTON_NONE_MODE(cBlnk)	((cBlnk[2]&0x0F)==0x02)
#define	IS_IBUTTON_NOT_NONE_MODE(cBlnk)	((cBlnk[2]&0x0F)!=0x02)

#define	GET_IBUTTON_START_ZERO_BASE_INDEX(cBlnk)	(cBlnk[0]>>4)
#define	GET_IBUTTON_END_ZERO_BASE_INDEX(cBlnk)	(0x0F&cBlnk[0])
#define	SET_IBUTTON_START_ZERO_BASE_INDEX(cBlnk,cZerBaseIndex)	(cBlnk[0] = (((unsigned char)((cZerBaseIndex<<4)&0xF0))+(0x0F&cBlnk[0])))
#define	SET_IBUTTON_END_ZERO_BASE_INDEX(cBlnk,cZerBaseIndex)	(cBlnk[0] = (((unsigned char)(cZerBaseIndex&0x0F))+(0xF0&cBlnk[0])))

/*
 * this structure is saved to flash
 */
typedef struct tagSYSINFO_STD{//the extension of SYSINFO

	unsigned char cBlank[SYSTEM_SIZE_BLANK];	//Don't use this member

	UINT32 dwSize;		//the size of this structure = sizeof(SYSINFO_STD)
...
typedef struct tagSYSINFO{
	...
	unsigned char sStrucVer[SYSTEM_SIZE_STRUCTVER];	//	 	: the version of structure 2.2.0.0
								//    	  sVersion[0]-marjor version (2)
								//		  sVersion[1]-minor version  (2)
								//		  sVersion[2]-fix mistake version  (0)
								// 		  sVersion[3]-build version (0)
								//default: sStrucVer[]={2,0,0,0}
...
typedef struct tagSYSINFO_OLD{
	...
	UINT32 dwSize;		//the size of this structure = sizeof(SYSINFO_OLD)
