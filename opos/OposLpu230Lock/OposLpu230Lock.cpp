...
STDAPI DllCanUnloadNow(void)
{
	...
	if( hResult == S_OK  ){
		...
		CSoLock::CShare::get_instance().remove_queue();

		CDll::get_instance().dll_off();
		CLog::GetLog()->Log( true, CLog::LEV_HIGH, _T("[DETAIL] Removed CSoLock.\n")  );
	}
	else{// it is dregon!
		if( _AtlModule.GetLockCount() == 0 ){//ORG == 1
			...
			CDll::get_instance().dll_off();
			CLog::GetLog()->Log( true, CLog::LEV_HIGH, _T("[DETAIL] Removed CSoLock(CNT==0).\n")  );
		}
	}
