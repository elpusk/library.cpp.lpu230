...
namespace _ns_tools
{
	...
	public:
		...
		ct_version(const T_EACH_VERSION_ITEM* p_version)
		{
			do {
		...
		ct_version(T_EACH_VERSION_ITEM major, T_EACH_VERSION_ITEM minor, T_EACH_VERSION_ITEM fix, T_EACH_VERSION_ITEM build)
		{
			m_major = major;	m_minor = minor;	m_fix = fix;	m_build = build;
		}

		ct_version(const ct_version& version)
		{
			*this = version;
		}

		virtual ~ct_version(void)
		{

		}

		ct_version& operator=(const ct_version& src)
		{
			m_major = src.m_major;	m_minor = src.m_minor;	m_fix = src.m_fix;	m_build = src.m_build;
			return *this;
		}

		bool operator > (const ct_version& rhs)
		{
			bool b_result(false);
		...
		bool set_by_string(const std::wstring& s_version)
		{
			...
			do {
				...
				for (size_t i = 0; i < array_version.size(); i++) {
					if (!_ns_tools::ct_convert::get_value(n_data, array_version[i], 10)) {
						b_error = true;
						break;
					}
