...
namespace _ns_tools
{
	...
	//exported methods
	public:
		static std::wstring get_unicode_printable_string(const _ns_tools::type_v_buffer& v_data)
		{
			std::wstring s;
			...
			do {
				...
				for (size_t i = 0; i < v_data.size(); i++) {
					if (std::isprint(v_data[i]) == 0) {
						if (v_data[i] != 0x00) {
							s.clear();
							break;
						}
						break;//null terminatred string
					}
					s.push_back((wchar_t)v_data[i]);
				}//end for
			} while (false);
			return s;
		}
		static std::string get_mbcs_printable_string(const _ns_tools::type_v_buffer& v_data)
		{
			std::string s;
			...
			do {
				...
				for (size_t i = 0; i < v_data.size(); i++) {
					...
					s.push_back((char)v_data[i]);
				}//end for
			} while (false);
			return s;
		}

		static std::wstring get_string_with_prepostfix(const _ns_tools::type_v_buffer& v_src, int n_base, const std::wstring& s_prefix , const std::wstring& s_postfix, const std::wstring& s_delimiter = L"")
		{
			std::wstring s_data;

			do {
				s_data = _ns_tools::ct_convert::get_string(v_src, n_base, s_prefix, s_delimiter);
				s_data += s_postfix;
			} while (false);
			return s_data;
		}

		static std::wstring get_string_with_prepostfix(const unsigned char *ps_src, size_t n_src, int n_base, const std::wstring& s_prefix, const std::wstring& s_postfix, const std::wstring& s_delimiter = L"")
		{
			std::wstring s_data;
			...
			do {
				if (ps_src == nullptr || n_src == 0)
					continue;
				_ns_tools::type_v_buffer v_src(&ps_src[0], &ps_src[n_src]);
				s_data = _ns_tools::ct_convert::get_string(v_src, n_base, s_prefix, s_delimiter);
				s_data += s_postfix;
			} while (false);
			return s_data;
		...
		static std::wstring get_string(const _ns_tools::type_v_buffer & v_src, int n_base, const std::wstring & s_prefix = L"", const std::wstring & s_delimiter = L"")
		{
			std::wstring s_out;
		...
		static bool get_value(unsigned long & dw_out, const std::wstring & s_src)
		{
			...
			do {
				...
				b_result = ct_convert::get_value(dw_out, s_char, n_base);
		...
		static bool get_value(unsigned char & c_out, const std::wstring & s_src)
		{
			...
			do {
				...
				b_result = ct_convert::get_value(c_out, s_char, n_base);
			} while (false);
			return b_result;
		}
		 ...
		 */
		static std::pair<bool, _ns_tools::type_v_buffer> get_bcd_code_from_decimal_stirng(const std::wstring & s_decimal, size_t n_bcd_size_by_byte_unit = 0, bool b_left_zero_padding = true)
		{
			std::wstring s_in(s_decimal);
			_ns_tools::type_v_buffer v_bcd(0), v_bcd_result(0);
			bool b_odd(true);
		...
		/**
			 ...
			 * \return
			 */
		static std::pair<bool, _ns_tools::type_v_buffer> get_bcd_code_from_decimal_stirng(
		...
		{
			bool b_result(false);
			_ns_tools::type_v_buffer v_bcd_result(0);
		...
		static std::pair<bool,_ns_tools::type_v_buffer> get_bcd_code(unsigned long n_in, size_t n_bcd_size_by_byte_unit = 0, bool b_left_zero_padding = true)
		{
		...
		static std::pair<bool, _ns_tools::type_v_buffer> get_bcd_code(const _ns_tools::type_v_buffer& v_ascii_numeric)
		{
			_ns_tools::type_v_buffer v_bcd_result(0);
			bool b_result(false);
		...
		static bool get_value_from_bcd_code(unsigned long& dw_out, const _ns_tools::type_v_buffer& v_bcd)
		{
			return get_value_from_bcd_code(dw_out, &v_bcd[0], (unsigned long)v_bcd.size());
		}

		//
		static const std::wstring & get_type_string(type_value_type type)
		{
			static std::wstring s_type(L"value_type_none");

			switch (type) {
			case value_type_int:	s_type = L"value_type_int";	break;
			case value_type_string:	s_type = L"value_type_string";	break;
			case value_type_hex_string:	s_type = L"value_type_hex_string";	break;
			case value_type_bool:	s_type = L"value_type_bool";	break;
			case value_type_bcd:	s_type = L"value_type_bcd";	break;
			case value_type_multi_hex_string: s_type = L"value_type_multi_hex_string";	break;
			default:	s_type = L"value_type_none";	break;
			}//end switch
			return s_type;
		}

		static void hex_string_from_binary(std::wstring & sOut, const _ns_tools::type_v_buffer & vIn, const std::wstring & s_delimiter = L"")
		{
			sOut.clear();

			for( size_t i = 0; i<vIn.size(); i++ ){//don't use for_each, for_each will be problemed in VC2010.
				std::wstring s_hex = ct_convert::get_string((unsigned char)vIn[i], 16);
				sOut += s_hex;
				if(i != (vIn.size()-1) )
					sOut += s_delimiter;
			}//end for
		}
		static std::wstring hex_string_from_binary(const _ns_tools::type_v_buffer& vIn, const std::wstring& s_delimiter = L"")
		{
			std::wstring s_out;
			hex_string_from_binary(s_out, vIn, s_delimiter);
			if (s_out.empty())
				s_out = L"";
			return s_out;
		}
		...
		static void hex_string_from_binary(std::wstring & sOut, const unsigned char *s_In, int n_In, const std::wstring & s_delimiter = L"")
		{
			do {
				...
				for (int i = 0; i < n_In; i++) {
					sOut += ct_convert::get_string(s_In[i], 16);
					if (i != (n_In - 1))
						sOut += s_delimiter;
				}//end for
		...
		static void hex_string_from_binary(std::string& sOut, const _ns_tools::type_v_buffer& vIn, const std::string& s_delimiter = "")
		{
			sOut.clear();

			for (size_t i = 0; i < vIn.size(); i++) {//don't use for_each, for_each will be problemed in VC2010.
				std::string s_hex = ct_convert::_get_string((unsigned char)vIn[i], 16);
				sOut += s_hex;
				if (i != (vIn.size() - 1))
					sOut += s_delimiter;
			}//end for
		...
		static void hex_string_from_binary(std::string& sOut, const unsigned char* s_In, int n_In, const std::string& s_delimiter = "")
		{
			do {
				...
				for (int i = 0; i < n_In; i++) {
					sOut += ct_convert::_get_string(s_In[i], 16);
					if (i != (n_In - 1))
						sOut += s_delimiter;
				}//end for
		...
		static void binary_from_hex_string(_ns_tools::type_v_buffer & vOut, const std::wstring & s_hex, const std::wstring & s_seperator = std::wstring(L""))
		{
				...
				if (s_seperator.empty()) {// no delimiter
					...
					while ((n_pos = s_src.find(s_seperator)) != std::wstring::npos) {
						if (n_pos == 0 && (s_seperator.compare(L" ") == 0)) {
							...
							ct_string::remove_white_space_of_prepostfix(s_token);
		...
		static void binary_from_hex_string(_ns_tools::type_v_buffer& vOut, const std::string& s_hex, const std::string& s_seperator = std::string(""))
		{
		...
		//
		static bool get_data(_ns_tools::type_v_buffer& vOut, const std::string& s_src, bool b_insert_null_to_tail = true)
		{
			bool b_result(false);
		...
		static bool get_data(_ns_tools::type_v_buffer& vOut, const std::wstring& s_src, bool b_insert_null_to_tail = true)
		{
			bool b_result(false);
		...
		static bool get_data(std::wstring & s_data, const std::wstring & s_src, const type_value_type type)
		{
			...
			do {
				switch (type) {
				case ct_convert::value_type_string:
				case ct_convert::value_type_hex_string:
				case ct_convert::value_type_int:
				case ct_convert::value_type_bool:
				case ct_convert::value_type_bcd:
				case ct_convert::value_type_multi_hex_string:
					s_data = s_src;
					break;
				default:
					continue;
		...
		static bool get_data(unsigned long& n_data, const std::wstring& s_src, const type_value_type type)
		{
			...
			do {
				std::wstring s_data;
				std::pair<bool, _ns_tools::type_v_buffer> pair_result;
				...
				case ct_convert::value_type_int:
				case ct_convert::value_type_bcd:
					n_data = (unsigned long)_wtol(s_src.c_str());
					if (errno == ERANGE)
						continue;
					break;
				case ct_convert::value_type_hex_string:
					ct_string::remove_all_char_except_hex_char(s_data, s_src);
					n_data = wcstoul(&s_data[0], nullptr, 16);
					if (errno == ERANGE)
						continue;
					break;
				case ct_convert::value_type_string:
		...
		static bool get_data(_ns_tools::type_v_buffer & v_data, const std::wstring & s_src, const type_value_type type)
		{
			bool b_result(false);
			std::pair<bool, _ns_tools::type_v_buffer> pair_result;
			...
			do {
				...
				case ct_convert::value_type_hex_string:
					ct_string::remove_all_char_except_hex_char(s_data, s_src);
					ct_convert::binary_from_hex_string(v_data, s_data);
					break;
				case ct_convert::value_type_bcd:
					pair_result = ct_convert::get_bcd_code_from_decimal_stirng(s_src);
		...
		static bool get_data(unsigned char& c_data, const std::wstring& s_src, const type_value_type type)
		{
			...
			do {
				...
				case ct_convert::value_type_bcd:
					...
					break;
				case ct_convert::value_type_bool:
					b_result = ct_convert::get_data(b_value, s_src, type);
					if (b_result) {
						if (b_value)
							c_data = 1;
						else
					...
					continue;
				case ct_convert::value_type_int:
					b_result = ct_convert::get_data(n_value, s_src, type);
					if (b_result) {
		...
		static bool get_data(type_v_ul_buffer & v_ul_data, const std::wstring& s_src, const type_value_type type, const std::wstring& s_separator)
		{
			...
			do {
				...
				_ns_tools::type_list_wstring list_s_token;
				_ns_tools::type_list_wstring::iterator it_token;
				unsigned long n_data(0);

				switch (type) {
				case ct_convert::value_type_multi_hex_string:
					ct_convert::tokenizer(list_s_token, s_data, s_separator);
					...
					for (; it_token != std::end(list_s_token); ++it_token) {
						if (!ct_convert::get_data(n_data, *it_token, ct_convert::value_type_hex_string)) {
							b_result = false;
							break;
						}
		...
		static long get_long_from_big_endian_byte(const _ns_tools::type_v_buffer & v_value)
		{
			return _ns_tools::ct_convert::get_long_from_big_endian_byte(&v_value[0], (int)v_value.size());
		}
		static long get_long_from_big_endian_byte(const unsigned char* ps_value, int n_value)
		{
			long n_out = 0;
		...
		static void change_endian(unsigned char *ps_data, int n_data)
		{
			if (ps_data == nullptr)
				return;
			if (n_data <= 0)
			...
			for (int i = 0; i < n_data; i++) {
				v_data.push_back(ps_data[n_data - 1 - i]);
			}//end for
			//
			memcpy(ps_data, &v_data[0], n_data);
		}

		static void change_endian(_ns_tools::type_v_buffer & v_data)
		{
			change_endian(&v_data[0], (int)v_data.size());
		}

		static void change_endian(unsigned long & n_out, const _ns_tools::type_v_buffer & v_in)
		{
			do {
				if (v_in.empty())
					continue;
				if (v_in.size() > sizeof(n_out))
					continue;

				unsigned char *ps_out = (unsigned char *)&n_out;
				for (size_t i = 0; i < v_in.size(); i++) {
					ps_out[v_in.size() - 1 - i] = v_in[i];
				}//end for
			} while (false);
		}

		static void change_endian(unsigned int & n_out, const _ns_tools::type_v_buffer & v_in)
		{
			do {
				if (v_in.empty())
					continue;
		...
		static void change_endian(unsigned short & w_out, const _ns_tools::type_v_buffer & v_in)
		{
			do {
				if (v_in.empty())
					continue;
				if (v_in.size() > sizeof(w_out))
					continue;

				unsigned char *ps_out = (unsigned char *)&w_out;
				for (size_t i = 0; i < v_in.size(); i++) {
					ps_out[v_in.size() - 1 - i] = v_in[i];
				}//end for
			} while (false);
		...
		*/
		static size_t change(wchar_t* p_ss_multi_dst, const _ns_tools::type_list_wstring& list_s_src)
		{
			size_t n_string(0);
			do {
		...
		* if p_ss_multi_dst isn't null, return the number of string in p_ss_multi_dst buffer.
		*/
		static size_t change(char *p_ss_multi_dst, const _ns_tools::type_list_string& list_s_src)
		{
			size_t n_string(0);
			do {
		...
		* if p_ss_multi_dst isn't null, return the number of string in p_ss_multi_dst buffer.
		*/
		static size_t change(wchar_t* p_ss_multi_dst, const _ns_tools::type_set_wstring & set_s_src)
		{
			size_t n_string(0);
			do {
		...
		static size_t change(_ns_tools::type_list_wstring & list_s_dst, const wchar_t *p_ss_multi_src)
		...
		static size_t change(_ns_tools::type_set_wstring & set_s_dst, const wchar_t *p_ss_multi_src)
		{//p_ss_multi_src is multi string
			const wchar_t *pDest;
			std::wstring stemp;
			size_t n_count = 0;
		...
		static size_t change(_ns_tools::type_list_string& list_s_dst, const char* p_ss_multi_src)
		{//p_ss_multi_src is multi string
			const char* pDest;
			std::string stemp;
			size_t n_count = 0;
		...
		static size_t change(_ns_tools::type_set_string& set_s_dst, const char* p_ss_multi_src)
		{//p_ss_multi_src is multi string
			const char* pDest;
			std::string stemp;
			size_t n_count = 0;
			...
			do {
				...
				while (p_ss_multi_src[nOffset] != NULL) {
					...
					pDest = &(p_ss_multi_src[nOffset]);
					stemp = pDest;
					std::pair<_ns_tools::type_set_string::iterator, bool> result = set_s_dst.insert(stemp);
		...
		static size_t change(_ns_tools::type_set_wstring& set_s_dst, unsigned long n_src, const unsigned char* s_src)
		{
			std::wstring stemp;
		...
		static size_t change(_ns_tools::type_set_wstring& set_s_dst, const _ns_tools::type_v_buffer&v_src)
		{
			size_t n_item(0);
			do {
				set_s_dst.clear();
				...
				if (v_src.empty())
					continue;
				n_item = _ns_tools::ct_convert::change(set_s_dst, (unsigned long)v_src.size(), &v_src[0]);
			} while (false);
			return n_item;
		}

		static size_t tokenizer(_ns_tools::type_list_wstring& list_s_token, const std::wstring& s_src, const std::wstring& s_separator, bool b_if_token_empty_then_ignore = true)
		{
			do {
				list_s_token.clear();
				//
		...
		static void convert(_ns_tools::type_v_string& v_string_out, const char* ss_stirg, size_t n_row, size_t n_column)
		{
			std::string s_row;

			do {
				v_string_out.resize(0);
				if (n_row == 0)
					continue;
				if (n_column == 0)
					continue;
				...
				for (size_t i = 0; i < n_row; i++) {
					...
					for (size_t j = 0; j < n_column; j++) {
						if (ss_stirg[i* n_column+j])
							s_row.push_back(ss_stirg[i * n_column + j]);
						else {
							v_string_out.push_back(s_row);
							break;//exit for j
						}
					}//end for j
				}//end for i
		 ...
		 */
		static size_t change_from_utf8_multistring(_ns_tools::type_list_wstring & list_s_dst, size_t & n_total_characters, const char *p_ss_multi_utf8, size_t n_ss_multi_utf8)
		{
			size_t n_item(0);
			...
			do{
				...
				for( ; it_info != std::end(v_pair_string); ++it_info){
					if( !_ns_tools::ct_operation::is_all_utf8(it_info->first,it_info->second) ){
						n_item = 0;
						break;//format error
					}
				}//end for
				...
				_ns_tools::type_v_ws_buffer v_ws_data(0);
				std::wstring s_data;

				for( ; it_info != std::end(v_pair_string); ++it_info){
					n_len = MultiByteToWideChar(CP_UTF8, 0, it_info->first, (int)strlen(it_info->first), NULL, NULL);
					if(n_len>0 ){
						v_ws_data.resize(n_len + 1, 0);	v_ws_data.assign(v_ws_data.size(), 0);
						MultiByteToWideChar(CP_UTF8, 0, it_info->first, (int)strlen(it_info->first), &v_ws_data[0], n_len);
						if( v_ws_data[0] ){
							s_data.clear();
							s_data = (const wchar_t*)(&v_ws_data[0]);
		 ...
		 * @return size_t - the number of string in list_s_dst.
		 */
		static size_t change_from_utf8_multistring(_ns_tools::type_deque_wstring & q_s_dst, size_t & n_total_characters, const char *p_ss_multi_utf8, size_t n_ss_multi_utf8)
		{
			_ns_tools::type_list_wstring list_s_dst;
			size_t n_string = _ns_tools::ct_convert::change_from_utf8_multistring(list_s_dst,n_total_characters,p_ss_multi_utf8,n_ss_multi_utf8);
			do{
				q_s_dst.clear();
				if(n_string == 0){
					continue;
		 ...
		 * @return size_t - the number of string in list_s_dst.
		 */
		static size_t change_from_utf8_multistring(_ns_tools::type_set_wstring & set_s_dst, size_t & n_total_characters, const char *p_ss_multi_utf8, size_t n_ss_multi_utf8)
		{
			_ns_tools::type_list_wstring list_s_dst;
			size_t n_string = _ns_tools::ct_convert::change_from_utf8_multistring(list_s_dst,n_total_characters,p_ss_multi_utf8,n_ss_multi_utf8);
			do{
		...
		static void get_euc_kr_string_from_unicode(_ns_tools::type_v_buffer& v_euc_kr_string, const std::wstring& s_unicode)
		{
			do {
				v_euc_kr_string.resize(0);
				//
				if (s_unicode.empty())
					continue;
				//
				int n_len = WideCharToMultiByte(CP_ACP, 0, s_unicode.c_str(), -1, NULL, 0, NULL, NULL);
				v_euc_kr_string.resize(n_len, 0);
				WideCharToMultiByte(CP_ACP, 0, s_unicode.c_str(), -1, (LPSTR)&v_euc_kr_string[0], n_len, NULL, NULL);
	...
	private:
		static std::string _get_string(const type_v_buffer& v_src, int n_base, const std::string& s_prefix = "", const std::string& s_delimiter = "")
		{
			...
			do {
				...
				for (size_t i = 0; i < v_src.size(); i++) {
					s_c = _get_string(v_src[i], n_base, s_prefix);
					if (i != v_src.size() - 1)
						s_out += (s_c + s_delimiter);
					else
						s_out += s_c;
	...
	private://don't call these methods
		ct_convert();
		ct_convert( const ct_convert & );
		ct_convert & operator=(const ct_convert &);
