...
namespace _mp {
		...
		*/
		bool cmain_ctl::_execute(cio_packet::type_ptr& ptr_request)
		{
			cio_packet response;
			...
			do {
				if (ptr_request->is_response()) {
					b_completet = true;
					response = *_generate_error_response(*ptr_request, cio_packet::error_reason_request_type);
					continue;
				}
				...
				if (n_data > cws_server::const_default_max_rx_size_bytes) {
					...
					b_completet = true;
					response = *_generate_error_response(*ptr_request, cio_packet::error_reason_overflow_buffer);
					continue;
				}
				...
				default:
					b_completet = true;
					response = *_generate_error_response(*ptr_request, cio_packet::error_reason_action_code);
					break;
				}//end switch
